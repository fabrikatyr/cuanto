...
app.service('fixedcostsService', function () {
    this.getfixedcosts = function () {
        return fixedcosts;
    };

    this.insertFixedcost = function (firstName, lastName, city) {
        var topID = fixedcosts.length + 1;
        fixedcosts.push({
            id: topID,
            firstName: firstName,
            lastName: lastName,
    ...
    this.deleteFixedcost = function (id) {
        for (var i = fixedcosts.length - 1; i >= 0; i--) {
            if (fixedcosts[i].id === id) {
